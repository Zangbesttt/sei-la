...
local C={V=false,Vel=80,VC=nil,NC=false,NCC=nil,G=nil,GC=nil,A=false,AC=nil,Teclas={Voo="L",Noclip="N",Grudar="K",Atacar="J"},Lista=false,Alvo=nil,GC2=nil}
local sg=Instance.new("ScreenGui")sg.Name="Painel"sg.ResetOnSpawn=false sg.Parent=LP:WaitForChild("PlayerGui")
local st=Instance.new("TextLabel")st.Size=UDim2.new(0,250,0,20)st.Position=UDim2.new(0,20,0,460)st.BackgroundColor3=Color3.fromRGB(5,5,15)st.BorderSizePixel=0 st.Text="✅ Painel carregado"st.TextColor3=Color3.fromRGB(150,255,150)st.TextSize=10 st.Font=Enum.Font.Gotham st.TextXAlignment=Enum.TextXAlignment.Left st.Parent=sg
local stc=Instance.new("UICorner")stc.CornerRadius=UDim.new(0,6)stc.Parent=st
local stp=Instance.new("UIPadding")stp.PaddingLeft=UDim.new(0,8)stp.Parent=st
local function log(m)st.Text=m end
...
local mc1=Instance.new("UICorner")mc1.CornerRadius=UDim.new(0,27)mc1.Parent=mini
local ms1=Instance.new("UIStroke")ms1.Color=Color3.fromRGB(0,150,255)ms1.Thickness=2.5 ms1.Parent=mini
local m=Instance.new("Frame")m.Size=UDim2.new(0,250,0,440)m.Position=UDim2.new(0,20,0,20)m.BackgroundColor3=Color3.fromRGB(0,0,0)m.BorderSizePixel=0 m.Active=true m.Draggable=true m.Parent=sg
local mc2=Instance.new("UICorner")mc2.CornerRadius=UDim.new(0,10)mc2.Parent=m
local ms2=Instance.new("UIStroke")ms2.Color=Color3.fromRGB(0,150,255)ms2.Thickness=2 ms2.Parent=m
mini.MouseButton1Click:Connect(function()m.Visible=true mini.Visible=false st.Visible=true end)
...
local mbc=Instance.new("UICorner")mbc.CornerRadius=UDim.new(0,5)mbc.Parent=minB
...
minB.MouseButton1Click:Connect(function()m.Visible=false mini.Visible=true st.Visible=false end)
local function mk(y,txt,ini,fn)
...
local k=Instance.new("TextButton")k.Size=UDim2.new(0,26,0,24)k.Position=UDim2.new(1,-30,0,y)k.BackgroundColor3=Color3.fromRGB(30,30,50)k.BorderSizePixel=0 k.Text=ini k.TextColor3=Color3.fromRGB(255,200,100)k.TextSize=10 k.Font=Enum.Font.GothamBold k.Parent=m
...
local pF=Instance.new("Frame")pF.Size=UDim2.new(0,260,0,380)pF.Position=UDim2.new(0,290,0,20)pF.BackgroundColor3=Color3.fromRGB(0,0,0)pF.BorderSizePixel=0 pF.Active=true pF.Draggable=true pF.Visible=false pF.Parent=sg
...
C.GC2=R.Heartbeat:Connect(function()if not C.Alvo then return end local h=gr()if not h then return end local ac=C.Alvo.Character local ar=ac and ac:FindFirstChild("HumanoidRootPart")if not ar then return end pcall(function()ar.CFrame=h.CFrame*CFrame.new(0,2,-3)end)end)end)
end
end
end
...
pB.MouseButton1Click:Connect(function()pF.Visible=not pF.Visible if pF.Visible then up()log("👥 Lista aberta")else log("👥 Lista fechada")end end)
...
cb.MouseButton1Click:Connect(function()C.V=false C.NC=false C.G=nil C.A=false pV()if C.NCC then C.NCC:Disconnect()end if C.GC then C.GC:Disconnect()end if C.AC then C.AC:Disconnect()end if C.GC2 then C.GC2:Disconnect()end sg:Destroy()end)
...
elseif k==C.Teclas.Atacar then tA()
elseif k=="P" then pF.Visible=not pF.Visible if pF.Visible then up()end end end)
log("✅ Painel + Players carregado!")
