--[[ PAINEL PRINCIPAL + TECLAS CONFIGURÁVEIS + MINIMIZAR ]]
...
--[[ LISTA DE PLAYERS - ATIVAR/DESATIVAR POR PLAYER ]]
local P=game:GetService("Players")local R=game:GetService("RunService")local LP=P.LocalPlayer
local LG=_G.LOG or function()end
local sg=Instance.new("ScreenGui")sg.Name="P2"sg.ResetOnSpawn=false sg.Parent=LP:WaitForChild("PlayerGui")
local m=Instance.new("Frame")m.Size=UDim2.new(0,280,0,400)m.Position=UDim2.new(0,290,0,20)m.BackgroundColor3=Color3.fromRGB(0,0,0)m.BorderSizePixel=0 m.Active=true m.Draggable=true m.Visible=false m.Parent=sg
local mc=Instance.new("UICorner")mc.CornerRadius=UDim.new(0,10)mc.Parent=m
local ms=Instance.new("UIStroke")ms.Color=Color3.fromRGB(0,150,255)ms.Thickness=2 ms.Parent=m
local tt=Instance.new("TextLabel")tt.Size=UDim2.new(1,0,0,28)tt.BackgroundColor3=Color3.fromRGB(10,10,20)tt.BorderSizePixel=0 tt.Text="👥 PLAYERS"tt.TextColor3=Color3.fromRGB(0,180,255)tt.TextSize=12 tt.Font=Enum.Font.GothamBold tt.Parent=m
local ttc=Instance.new("UICorner")ttc.CornerRadius=UDim.new(0,10)ttc.Parent=tt
local xB=Instance.new("TextButton")xB.Size=UDim2.new(0,22,0,22)xB.Position=UDim2.new(1,-26,0,3)xB.BackgroundColor3=Color3.fromRGB(60,20,20)xB.BorderSizePixel=0 xB.Text="X"xB.TextColor3=Color3.fromRGB(255,120,120)xB.TextSize=12 xB.Font=Enum.Font.GothamBold xB.Parent=m
local xBc=Instance.new("UICorner")xBc.CornerRadius=UDim.new(0,5)xBc.Parent=xB
local sF=Instance.new("ScrollingFrame")sF.Size=UDim2.new(1,-16,1,-50)sF.Position=UDim2.new(0,8,0,36)sF.BackgroundTransparency=1 sF.BorderSizePixel=0 sF.ScrollBarThickness=4 sF.Parent=m
local ll=Instance.new("UIListLayout")ll.Padding=UDim.new(0,3)ll.Parent=sF
local G={Alvo=nil,Conn=nil}
local function gr()local c=LP.Character return c and c:FindFirstChild("HumanoidRootPart")end
local function up()
...
local r=Instance.new("Frame")r.Size=UDim2.new(1,-8,0,28)r.BackgroundColor3=p==LP and Color3.fromRGB(30,50,30)or Color3.fromRGB(25,25,35)r.BorderSizePixel=0 r.Parent=sF
...
tb.MouseButton1Click:Connect(function()local h=gr()local a=p.Character and p.Character:FindFirstChild("HumanoidRootPart")if h and a then pcall(function()h.CFrame=a.CFrame*CFrame.new(0,3,-5)end)LG("📍 TP -> "..p.Name)end end)
...
if G.Alvo==p then if G.Conn then G.Conn:Disconnect()end G.Alvo=nil gb.Text="🧲"gb.BackgroundColor3=Color3.fromRGB(70,30,90)LG("🛑 Soltou "..p.Name)return end
G.Alvo=p gb.Text="✓"gb.BackgroundColor3=Color3.fromRGB(30,90,30)LG("🧲 Grudou "..p.Name)
if G.Conn then G.Conn:Disconnect()end
G.Conn=R.Heartbeat:Connect(function()if not G.Alvo then return end local h=gr()if not h then return end local ac=G.Alvo.Character local ar=ac and ac:FindFirstChild("HumanoidRootPart")if not ar then return end pcall(function()ar.CFrame=h.CFrame*CFrame.new(0,2,-3)end)end)end)
end
end
end
...
up()
_G.TogglePlayers=function()m.Visible=not m.Visible if m.Visible then up()LG("👥 Lista aberta")else LG("👥 Lista fechada")end end
xB.MouseButton1Click:Connect(function()m.Visible=false LG("👥 Lista fechada")end)
LG("✅ Lista de players carregada!")
