...
local m=Instance.new("Frame")m.Size=UDim2.new(0,250,0,290)m.Position=UDim2.new(0,20,0,20)m.BackgroundColor3=Color3.fromRGB(0,0,0)m.BorderSizePixel=0 m.Active=true m.Draggable=true m.Parent=sg
...
cb.MouseButton1Click:Connect(function()C.V=false C.NC=false C.G=nil C.A=false pV()if C.NCC then C.NCC:Disconnect()end if C.GC then C.GC:Disconnect()end if C.AC then C.AC:Disconnect()end sg:Destroy()end)
...
log("🔑 L=Voo, N=Noclip, K=Grudar, J=Atacar")
